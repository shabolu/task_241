...
Шебанг — это первые символы в начале файла сценария вида #!path/to/interpreter, которые указывают ядру, каким интерпретатором выполнять файл (bash, sh, python и т. д.). Например: `#!/usr/bin/env bash`

2) Обязательно ли исполняемому файлу иметь расширение?

Нет. В Linux расширение не обязательно. Исполняемость определяется правами (chmod +x) и шебангом (или явным запуском через интерпретатор: bash ./скрипт). Расширение .sh - всего лишь соглашение, удобное для людей и IDE.
...
Включены флаги set -euo pipefail для удобной отладки:
- e: завершить скрипт при любой ошибке команды
- u: ошибка при обращении к неинициализированной переменной
- o pipefail: ошибка пайплайна, если ошибка в любой из команд в конвейере
